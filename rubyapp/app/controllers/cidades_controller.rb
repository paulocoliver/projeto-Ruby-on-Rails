class CidadesController < ApplicationController
  ...
  def index
    ...
    @cidades = Cidade.joins(:estado).select("cidades.*, estados.descricao as estado_descricao").page(params[:page]).per(50).order('estado_descricao, descricao')
  ...
  # GET /cidades/1.json
  def show
    @cidade = Cidade.joins(:estado).select("cidades.*, estados.descricao as estado_descricao").where(id: params[:id]).first
  ...
  # GET /load_cidades?id_estado=
  def load_cidades
    @cidades = Cidade.select("cidades.*").where(estado_id: params[:estado_id])
